...
int main() {
    ...
    constexpr auto double_fn = compile<double_it>();
    ...
    constexpr auto square_sum = compile<square_sum_expr>();
    ...
    constexpr auto nested_fn = compile<nested_expr>();
    ...
    constexpr auto clamped =
        MCond(raw < 0.0, Expr::lit(0.0), MCond(raw > 1.0, Expr::lit(1.0), raw));
    constexpr auto smoothstep_expr =
        let_("t", clamped, t * t * (3.0 - 2.0 * t));
    constexpr auto smoothstep = compile<smoothstep_expr>();
    ...
    constexpr auto pi_squared = compile<const_expr>();
