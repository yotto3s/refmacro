...
TEST(ConstraintSet, AddSingle) {
    ...
    constexpr auto cs =
        ConstraintSet<>{}.add(formula, "value must be positive");
...
TEST(ConstraintSet, AddMultiple) {
    constexpr auto cs =
        ConstraintSet<>{}
            .add(Expression<128>::var("#v") > Expression<128>::lit(0),
                 "positive")
            .add(Expression<128>::var("#v") < Expression<128>::lit(100),
                 "bounded");
...
TEST(ConstraintSet, MergeTwoSets) {
    constexpr auto cs1 = ConstraintSet<>{}.add(
        Expression<128>::var("x") > Expression<128>::lit(0), "x positive");
    constexpr auto cs2 =
        ConstraintSet<>{}
            .add(Expression<128>::var("y") > Expression<128>::lit(0),
                 "y positive")
            .add(Expression<128>::var("y") < Expression<128>::lit(10),
                 "y bounded");
...
TEST(ConstraintSet, AddReturnsNewSet) {
    ...
    constexpr auto cs1 =
        cs0.add(Expression<128>::var("x") > Expression<128>::lit(0), "pos");
    constexpr auto cs2 =
        cs1.add(Expression<128>::var("y") > Expression<128>::lit(0), "pos2");
...
TEST(ConstraintSet, ArithmeticFormula) {
    // x + y > 0
    constexpr auto formula =
        (Expression<128>::var("x") + Expression<128>::var("y")) >
...
TEST(ConstraintSet, LongOriginTruncated) {
    ...
    constexpr auto cs = ConstraintSet<>{}.add(
        Expression<128>::var("x") > Expression<128>::lit(0),
        "this is a very long origin string that should be truncated at 63 "
...
TEST(ConstraintSet, StructuralType) {
    ...
    []<auto C>() { static_assert(C.count == 1); }.template operator()<cs>();
}
