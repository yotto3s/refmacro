...
// Given lower bound (var has positive coeff) and upper bound (var has negative coeff),
// produce a new inequality without that variable.
consteval LinearInequality combine_bounds(
    const LinearInequality& lower, double lower_coeff,
    const LinearInequality& upper, double upper_abs_coeff,
    int var_id) {

    // After normalization: lower gives  x >= (-other_lower_terms - lower.constant)
    //                      upper gives  x <= ( other_upper_terms + upper.constant)
    // Combined: multiply lower by upper_abs_coeff, upper by lower_coeff, then add.
    // This cancels the target variable and preserves the direction (>= 0).

    LinearInequality result{};
    result.strict = lower.strict || upper.strict;

    // Scale and merge terms from both inequalities, skipping the eliminated variable
    // We collect terms, combining coefficients for the same var_id.

    // First, add scaled terms from lower
    for (std::size_t i = 0; i < lower.term_count; ++i) {
        if (lower.terms[i].var_id == var_id) continue;
    ...
    for (std::size_t i = 0; i < upper.term_count; ++i) {
        if (upper.terms[i].var_id == var_id) continue;
...
// then combines each (lower, upper) pair to produce a new system without var_id.
    ...
    for (std::size_t li = 0; li < lower_count; ++li) {
        for (std::size_t ui = 0; ui < upper_count; ++ui) {
            auto combined = combine_bounds(
                ...
                sys.ineqs[upper_idx[ui]], upper_abs_coeff[ui],
                var_id);
